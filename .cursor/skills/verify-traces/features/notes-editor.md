...
- **Alias.** A preview link written `[[Verify Beta|the other note]]` shows `the other note` as its text and clicking it opens Beta. `a.md-wiki-link[data-wiki-target="Verify Beta"]` matches the plain `[[Verify Beta]]` first when the note has both. A single newline is a soft break, so both anchors share one `<p>`; target the alias with `node helpers/drive.mjs click --selector '.md-preview a.md-wiki-link:nth-of-type(2)'` after `eval` confirms that selector's `innerText` is `the other note`.
...
- A new note is created next to the last note you opened, even after its tab is closed. `Close tab` does not clear the active file; only deleting it, or a fresh session with no note opened yet, falls back to the vault root. After opening `Verify Sub/Verify Index` and closing every tab, a create from the empty editor landed at `Verify Sub/Verify Eta.md`. Baseline seeds sit at the vault root, so `Verify Gamma.md` is the usual path when you have only opened root notes.
- CodeMirror does not update if you set `innerText` from `eval`. Use `type` or real key events. `type --focus-editor` inserts at the current cursor, usually the start of the file, so the text lands in front of `# Title` and turns the heading line into plain text. That still proves autosave. `type --selector ".cm-line:last-child"` clicks the right edge of the last line's last wrapped row and appends; a center click would split a line wider than half the editor.
- Auto-save is 800 ms. Heading rename is 1500 ms and keys off the first line anywhere in the note that starts with `# `, not only line 1. A glued heading (`glued # Verify Epsilon`) stops the rename until another `# ` line exists; appending `# Verify Zeta` as the last line renamed that file to `Verify Zeta.md`. Assert disk, not only the tab label.
- `.cm-line` innerText hides the `[[` `]]` of a wiki-link widget (`Missing: Verify Missing`). Read raw markdown with `read-vault`, not from the editor DOM.
